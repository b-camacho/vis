...
function SetEvenWeights(domains) {
	return domains.map(el => {
		el.weight = 1 / domains.length;
		return el
	});
}
...
$(document).ready(function () {
	// DOMAINS = SetEvenWeights(DOMAINS)
	var articles = AssignWorksToDomains([], DOMAINS);
	...
	var radius = height / 2.5,
		radiusIncluded = radius * 0.9,
		radiusExcluded = radius * 0.1,
		angularWidth = 0.05;
	//test data: [{domains: [d1Idx, d2Idx, d3Idx]},{domains: [d1Idx]}...]
	var testData = [
		{
			domains: [{idx: 1, v: 1}]
		},
		...
		{
			domains: [{idx:1 , v: 0.8}, {idx: 3, v:0.2}]
		},
		{
			domains: [{idx: 4,v:0.4},{idx: 7, v: 0.6}]
		}
	];

	const ScaleR = d3.scaleLinear()
		.domain([1, 0])
		.range([radiusIncluded, radiusExcluded]);

	let lineData = testData.map(({domains}) => {
		let points = [],
		colors = [],
		prevSelected = null;

		angleBounds
			.forEach((domAngles, idx) => {
			var centerAngle = (domAngles.angleBounds.begin + domAngles.angleBounds.end) / 2 - Math.PI / 2;
			let thisDom = domains.find(e => e.idx === idx);
				if (thisDom) {
					// if(prevSelected !== null) {
					// 	// points.push(PolarToCartesian(prevSelected - 0.1/*(centerAngle + prevSelected) / 2 - Math.PI / 2*/, radiusExcluded * 1.5))
					// 	points.push(PolarToCartesian(prevSelected, radiusExcluded * 1.5))
					// 	// points.push(PolarToCartesian(prevSelected + 0.1/*(centerAngle + prevSelected) / 2 - Math.PI / 2*/, radiusExcluded * 1.5))
					// }
					colors.push(DOMAINS[idx].hue);

					prevSelected = domAngles.angleBounds.end;
					let r = ScaleR(thisDom.v);
					points.push(PolarToCartesian(centerAngle - angularWidth, r));
					points.push(PolarToCartesian(centerAngle, r));
					points.push(PolarToCartesian(centerAngle + angularWidth, r));
				}
				else {
					points.push(PolarToCartesian(centerAngle, ScaleR(0)));
		...
		let hue = colors.length >= 2 ? d3.interpolateLab(colors[0], colors[1])(0.5) : colors[0];
		return { points, hue }
			// var domsCartesian = domains
			// 	.map(domain => {
			// 		return (angleBounds[domain].angleBounds.begin + angleBounds[domain].angleBounds.end) / 2 - Math.PI / 2
			// 	})
			// 	.map(angle => PolarToCartesian(angle, radius));
			//
			// domsCartesian.push({
			// 	x: 0,
			// 	y: 0,
	...
	lineData.forEach(singleLineData => {
			lineGroup.append('path').datum(singleLineData.points)
			// .attr("class", "line")
				.attr("d", line)
				// .attr("stroke", "#ff0000")
				.attr("stroke-width", 2)
				.attr("fill", singleLineData.hue)
				.attr("opacity", 0.2)
...
function GetDisciplineAngleBounds(angleBounds, works, minUnit) {
	...
	works.forEach(function (work) {
		...
		else if (!(0 <= domainToDiscliplinesMap[work.domains[0].name].indexOf(work.discipline)))
...
function DrawDomains(articles, angleBounds) {
	...
	for (var cros in crossings)
		if (crossings.hasOwnProperty(cros)) {
			var angle1 = domainCoordinates[crossings[cros].domain1].angle,
				angle2 = domainCoordinates[crossings[cros].domain2].angle
			if (angle1 > angle2) {
				var tmp = angle1;
				angle1 = angle2;
				angle2 = tmp;
	...
	var nodeTip = d3.tip().attr('class', 'd3-tip').html(function (d) {
		if (d.topic)
			...
			return d.domains.map(function (dom) {
					return jsStrings.vis.domains[dom.name]
				}).join("; ") +
